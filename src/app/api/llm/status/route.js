...
export async function GET() {
  try {
    ...
    const calcStats = (logs) => {
      if (!logs || logs.length === 0) return { count: 0, avgDuration: 0, errorCount: 0 };
      const successLogs = logs.filter(l => l.level === 'info' && l.duration_ms);
      const avgDuration = successLogs.length > 0
        ? successLogs.reduce((sum, l) => sum + l.duration_ms, 0) / successLogs.length
        : 0;
      const errorCount = logs.filter(l => l.level === 'error').length;
      return { count: logs.length, avgDuration: Math.round(avgDuration), errorCount };
    };
